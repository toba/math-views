...
public class MathListIndex {
    ...
    func prevIndex() -> MathListIndex? {
        if self.subIndexType == .none {
            if self.atomIndex > 0 {
                return MathListIndex(level0Index: self.atomIndex - 1)
            }
        } else {
            if let prevSubIndex = self.subIndex?.prevIndex() {
                return MathListIndex(at: self.atomIndex, with: prevSubIndex, type: self.subIndexType)
            }
        }
        return nil
    }
    ...
    func nextIndex() -> MathListIndex {
        if self.subIndexType == .none {
            ...
            return MathListIndex(at: self.atomIndex + 1, with: self.subIndex, type: self.subIndexType)
        } else {
            return MathListIndex(at: self.atomIndex, with: self.subIndex?.nextIndex(), type: self.subIndexType)
        }
    }
    ...
    func levelUp(with subIndex: MathListIndex?, type: MathListSubIndexType) -> MathListIndex {
        if self.subIndexType == .none {
            return MathListIndex(at: self.atomIndex, with: subIndex, type: type)
        }
        
        return MathListIndex(at: self.atomIndex, with: self.subIndex?.levelUp(with: subIndex, type: type), type: self.subIndexType)
    }
    ...
    func levelDown() -> MathListIndex? {
        ...
        if let subIndexDown = self.subIndex?.levelDown() {
            return MathListIndex(at: self.atomIndex, with: subIndexDown, type: self.subIndexType)
        } else {
            return MathListIndex(level0Index: self.atomIndex)
        }
    }
    ...
    public convenience init(at location: Int, with subIndex: MathListIndex?, type: MathListSubIndexType) {
