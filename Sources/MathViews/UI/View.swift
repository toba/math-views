...
/// TeX math rendering style for `MathView`.
///
/// Controls how fractions and limits on large operators are rendered.
/// Display style uses full-size fractions and stacked limits; text style
/// uses inline fractions and side-set limits.
public enum RenderingStyle {
    /// Display style — equivalent to `$$…$$` or `\[…\]` in TeX.
    case display
    /// Text (inline) style — equivalent to `$…$` or `\(…\)` in TeX.
    case text
}
...
public struct MathView: View {
    ...
    private var renderingStyle: RenderingStyle = .display
    ...
    /// Set the rendering style (display or text).
    public func renderingStyle(_ style: RenderingStyle) -> MathView {
        var copy = self
        copy.renderingStyle = style
        return copy
    }
    ...
    private func render() -> Result<RenderInfo, any Error> {
        ...
        let height =
            displayList.ascent + displayList.descent + contentInsets.top
                + contentInsets
                .bottom
        ...
        let textY =
            (availableHeight - contentHeight) / 2 + displayList.descent
                + contentInsets
                .bottom
