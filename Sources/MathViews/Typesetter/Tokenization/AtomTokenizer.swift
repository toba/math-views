...
/// Converts a ``MathList``'s atoms into a flat sequence of ``BreakableElement`` values.
///
/// Each atom becomes one or more elements with pre-calculated widths and break-quality
/// scores. Complex atoms (fractions, radicals, large operators) are pre-rendered into
/// ``Display`` objects by ``DisplayPreRenderer`` so their widths can be measured before
/// line fitting. Scripts (super/subscripts) are grouped with their base element so they
/// aren't separated across lines.
final class AtomTokenizer {
    // MARK: - Fallback Metrics

    /// Approximate ascent as a fraction of font size, used when the MATH table is unavailable.
    private static let fallbackAscentRatio: CGFloat = 0.5
    /// Approximate descent as a fraction of font size for line-break element sizing.
    private static let fallbackDescentRatio: CGFloat = 0.2
    /// Approximate ascent for delimiter elements as a fraction of font size.
    private static let delimiterAscentRatio: CGFloat = 0.6
    ...
    private func tokenizeAtom(
        ...
        switch atom.type {
            ...
            case .overbrace:
                guard let ob = atom as? OverBrace else { return nil }
                return tokenizeOverBrace(ob, prevAtom: prevAtom, atomIndex: atomIndex)

            case .underbrace:
                guard let ub = atom as? UnderBrace else { return nil }
                return tokenizeUnderBrace(ub, prevAtom: prevAtom, atomIndex: atomIndex)
    ...
    private func tokenizeTextAtom(
        ...
        let ascent = font.mathTable?.axisHeight ?? font.fontSize * Self.fallbackAscentRatio
        let descent = font.fontSize * Self.fallbackDescentRatio
        ...
        -> BreakableElement?
    {
        ...
        let ascent = font.fontSize * Self.fallbackAscentRatio
        let descent = font.fontSize * Self.fallbackDescentRatio
        let height = ascent + descent
        ...
        -> BreakableElement?
    {
        ...
        let ascent = font.fontSize * Self.delimiterAscentRatio
        let descent = font.fontSize * Self.fallbackDescentRatio
    ...
    private func tokenizeOverBrace(_ atom: OverBrace, prevAtom _: MathAtom?, atomIndex _: Int)
        -> BreakableElement?
    {
        let typesetter = Typesetter(withFont: font, style: style, cramped: cramped, spaced: false)
        guard let display = typesetter.makeOverBrace(atom) else { return nil }
        return makeDisplayElement(display, atom: atom)
    }

    private func tokenizeUnderBrace(_ atom: UnderBrace, prevAtom _: MathAtom?, atomIndex _: Int)
        -> BreakableElement?
    {
        let typesetter = Typesetter(withFont: font, style: style, cramped: cramped, spaced: false)
        guard let display = typesetter.makeUnderBrace(atom) else { return nil }
        return makeDisplayElement(display, atom: atom)
    }
