...
struct MathListBuilderSymbolTests {
    ...
    @Test func greekVariants() throws {
        ...
        for variant in variants {
            ...
            let unwrappedList = try #require(list, "Should parse \\\(variant)")
            #expect(error == nil, "Should not error on \\\(variant): \(error?.localizedDescription ?? "none")")
            #expect(unwrappedList.atoms.count >= 1, "\\\(variant) should have at least one atom")
        }
    }
    ...
    @Test func varsigmaCorrectUnicode() throws {
        ...
        let unwrappedList = try #require(list, "Should parse \\varsigma")
        #expect(error == nil)
        ...
        #expect(atom.nucleus == "\u{03C2}", "varsigma should map to U+03C2 (final sigma \u{03C2}), not U+03C1 (rho \u{03C1})")
    }
    ...
    @Test func slantedInequalities() throws {
        ...
        for ineq in inequalities {
            ...
            let unwrappedList = try #require(list, "Should parse \\\(ineq)")
            #expect(error == nil, "Should not error on \\\(ineq): \(error?.localizedDescription ?? "none")")
    ...
    @Test func diamondOperator() throws {
        ...
        let unwrappedList = try #require(list, "Should parse \\diamond")
        #expect(error == nil, "Should not error on \\diamond")
    ...
    @Test func hebrewLetters() throws {
        ...
        for letter in letters {
            ...
            let unwrappedList = try #require(list, "Should parse \\\(letter)")
            #expect(error == nil, "Should not error on \\\(letter): \(error?.localizedDescription ?? "none")")
            #expect(unwrappedList.atoms.count == 1, "\\\(letter) should have exactly one atom")
    ...
    @Test func miscSymbols() throws {
        ...
        for symbol in symbols {
            ...
            let unwrappedList = try #require(list, "Should parse \\\(symbol)")
            #expect(error == nil, "Should not error on \\\(symbol): \(error?.localizedDescription ?? "none")")
            #expect(unwrappedList.atoms.count >= 1, "\\\(symbol) should have at least one atom")
        }
    }
    ...
    @Test func mathbbCommand() throws {
        ...
        for letter in letters {
            ...
            let unwrappedList = try #require(list, "Should parse \\mathbb{\(letter)}")
            #expect(error == nil, "Should not error on \\mathbb{\(letter)}: \(error?.localizedDescription ?? "none")")
            #expect(unwrappedList.atoms.count == 1, "\\mathbb{\(letter)} should have one atom")
    ...
    @Test func bigDelimiterCommands() throws {
        ...
        for (command, expectedMultiplier) in sizeCommands {
            ...
            let unwrappedList = try #require(list, "Should parse \\\(command)(")
            #expect(error == nil, "Should not error on \\\(command)(: \(error?.localizedDescription ?? "none")")
            #expect(unwrappedList.atoms.count == 1, "\\\(command)( should have one atom")
            ...
            #expect(inner.delimiterHeight == expectedMultiplier, "Delimiter multiplier for \\\(command) should be \(expectedMultiplier)")
        }
    }
    ...
    @Test func bigDelimiterLeftRightVariants() throws {
        ...
        for (command, delim, expectedMultiplier) in variants {
            ...
            let unwrappedList = try #require(list, "Should parse \\\(command)\(delim)")
            #expect(error == nil, "Should not error on \\\(command)\(delim): \(error?.localizedDescription ?? "none")")
            #expect(unwrappedList.atoms.count == 1, "\\\(command)\(delim) should have one atom")
            ...
            #expect(inner.leftBoundary != nil, "Should have left boundary")
            #expect(inner.delimiterHeight != nil, "Should have explicit delimiter height")
            #expect(inner.delimiterHeight == expectedMultiplier, "Delimiter multiplier should be \(expectedMultiplier)")
        }
    }
    ...
    @Test func bigDelimiterMiddleVariants() throws {
        ...
        for (command, delim, expectedMultiplier) in variants {
            ...
            let inner = unwrappedList.atoms[0] as! Inner
            #expect(inner.delimiterHeight == expectedMultiplier, "Middle delimiter multiplier should be \(expectedMultiplier)")
        }
    }
    ...
    @Test func bigDelimiterMissingDelimiter() throws {
        ...
        if let error {
            #expect(ParseErrorCase.missingDelimiter.matches(error), "Error should be missingDelimiter")
        }
    }
    ...
    @Test func bigDelimiterInvalidDelimiter() throws {
        ...
        if let error {
            #expect(ParseErrorCase.invalidDelimiter.matches(error), "Error should be invalidDelimiter")
        }
    }
    ...
    @Test func bigDelimiterInExpression() throws {
        ...
        let unwrappedList = try #require(list, "Should parse expression with \\big delimiters")
        #expect(error == nil, "Should not produce an error")

        // Should have: inner (big(), x, +, y, inner big)
        #expect(unwrappedList.atoms.count == 5, "Should have 5 atoms")

        // First atom should be inner with big(
        let firstInner = unwrappedList.atoms[0] as! Inner
        #expect(firstInner.leftBoundary?.nucleus == "(")
        #expect(firstInner.delimiterHeight == 1.0)

        // Last atom should be inner with big)
        let lastInner = unwrappedList.atoms[4] as! Inner
    ...
    @Test func negatedInequalityRelations() throws {
        ...
        for (command, unicode) in symbols {
            ...
            let unwrappedList = try #require(list, "Should parse \\\(command)")
            #expect(error == nil, "Should not error on \\\(command)")
            #expect(unwrappedList.atoms.count == 1)
            #expect(unwrappedList.atoms[0].type == .relation)
            #expect(unwrappedList.atoms[0].nucleus == unicode, "\\\(command) should have unicode \(unicode)")
        }
    }
