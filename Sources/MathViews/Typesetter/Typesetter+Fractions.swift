...
extension Typesetter {
  ...
  func makeFraction(_ fraction: Fraction?) -> Display? {
    ...
    let resolvedDenominator = denominatorDisplay ?? MathListDisplay(displays: [], range: 0..<0)
    ...
    if fraction.hasRule {
      ...
      let distanceFromDenominatorToBar =
        (barLocation - barThickness / 2) - (resolvedDenominator.ascent - denominatorShiftDown)
    ...
    let display = FractionDisplay(
      numerator: resolvedNumerator, denominator: resolvedDenominator, position: currentPosition,
