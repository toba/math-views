...
`MathViews` provides a full Swift implementation of [iosMath](https://travis-ci.org/kostub/iosMath) 
...
Please also check out the example code below for examples of how to use `MathViews`
from SwiftUI.  

`MathViews` is similar to [MathJax](https://www.mathjax.org) or
...
`MathViews` is a Swift translation of the latest `iosMath` v0.9.5 release but includes bug fixes
...
`MathViews` prepackages everything needed for direct access via the Swift Package Manager.
...
## Requirements
`MathViews` works on iOS 11+ or MacOS 12+. It depends
...
`MathViews` is available from [toba/math-views](https://github.com/toba/math-views).
To use it in your code, just add the https://github.com/toba/math-views path to
Xcode's package manager.

## Usage

The library provides a class `MathUILabel` which is a `UIView` that
supports rendering math equations. To display an equation simply create
an `MathUILabel` as follows:
...
```
Adding `MathUILabel` as a sub-view of your `UIView` will render the
quadratic formula example shown above.

The following code creates a SwiftUI component called `MathView` encapsulating the MathUILabel:
...
struct MathView: UIViewRepresentable {
    ...
    var textAlignment: MathTextAlignment = .center
    var fontSize: CGFloat = 30
    var labelMode: MathUILabelMode = .text
    var insets: MathEdgeInsets = MathEdgeInsets()

    func makeUIView(context: Context) -> MathUILabel {
        let view = MathUILabel()
        view.setContentHuggingPriority(.required, for: .vertical)
        view.setContentCompressionResistancePriority(.required, for: .vertical)
        return view
    }

    func updateUIView(_ view: MathUILabel, context: Context) {
        view.latex = equation
        let font = FontManager().font(withName: font.rawValue, size: fontSize)
        font?.fallbackFont = UIFont.systemFont(ofSize: fontSize)
        view.font = font
        view.textAlignment = textAlignment
        view.labelMode = labelMode
        view.textColor = MathColor(Color.primary)
        view.contentInsets = insets
        view.invalidateIntrinsicContentSize()
    }

    func sizeThatFits(_ proposal: ProposedViewSize, uiView: MathUILabel, context: Context) -> CGSize? {
...
struct MathView: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> MathUILabel {
        let view = MathUILabel()
        view.setContentHuggingPriority(.required, for: .vertical)
        view.setContentCompressionResistancePriority(.required, for: .vertical)
        return view
    ...
    func updateNSView(_ view: MathUILabel, context: Context) {
        view.latex = equation
        let font = FontManager().font(withName: font.rawValue, size: fontSize)
    ...
    func sizeThatFits(_ proposal: ProposedViewSize, nsView: MathUILabel, context: Context) -> CGSize? {
        // Enable line wrapping by passing proposed width to the label
        if let width = proposal.width, width.isFinite, width > 0 {
...
`MathViews` supports automatic line wrapping (multiline display) for mathematical content. The implementation uses **interatom line breaking** which breaks equations at atom boundaries (between mathematical elements) rather than within them, preserving the semantic structure of the mathematics.

#### Using Line Wrapping with UIKit/AppKit

For direct `MathUILabel` usage, set the `preferredMaxLayoutWidth` property:
...
MathViews implements **two complementary line breaking mechanisms**:
...
`MathViews` now supports all standard LaTeX math delimiters for both inline and display modes. The parser automatically detects and handles these delimiters:
...
let (mathList, style) = MathListBuilder.buildWithStyle(fromString: "\\[x^2 + y^2 = z^2\\]")
...
if let mathList = mathList {
    let display = Typesetter.createLineForMathList(mathList, font: myFont, style: style)
...
Note: MathViews only supports the commands in LaTeX's math mode. There is
...
1) Find a math font compatible with `MathViews` that contains all the glyphs
for that language.
2) Add support to `MathViews` for standard Unicode fonts that contain all
language glyphs.
...
See the SwiftUI examples above for how to use `MathViews` as a Swift package dependency.

### Advanced configuration

`MathUILabel` supports some advanced configuration options:

##### Math mode

You can change the mode of the `MathUILabel` between Display Mode
...
```swift
label.font = FontManager.fontmanager.termesFont(withSize:20)
```
...
required for an `.otf` font to work with `MathViews`.  If you generate
...
```swift
let mathFont = FontManager().font(withName: MathFont.latinModernFont.rawValue, size: 30)
...
```swift
MathAtomFactory.addLatexSymbol("lcm", value: MathAtomFactory.operator(withName: "lcm", limits: false))
```

This creates an `\lcm` command that can be used in the LaTeX.

##### Content Insets
The `MathUILabel` has `contentInsets` for finer control of placement of the
...
If the LaTeX text given to `MathUILabel` is
...
`MathViews` is available under the MIT license. See the [LICENSE](./LICENSE)
file for more info.
