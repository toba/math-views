...
@MainActor struct MathUILabelLineWrappingTests {
  ...
  @Test func accentedCharactersWithLineWrapping() {
    ...
    #expect(
      constrainedSize.width < unconstrainedSize.width, "Constrained width should be < unconstrained"
    )
    #expect(
                ...
                let currIndex = allDisplays.firstIndex(where: { $0 === firstDisplay.display })
              {
                ...
                for i in 0..<allDisplays.count {
                  ...
                  {  // Same line
                    ...
                    if text.trimmingCharacters(in: .letters).contains(where: \.isWhitespace) {
  ...
  @Test func unicodeWordBoundaryRules() {
    ...
    func testOurWordBoundary(
      ...
      func hasWordBoundaryBetween(_ text1: String, and text2: String) -> Bool {
        ...
        combined.enumerateSubstrings(in: combined.startIndex..<combined.endIndex, options: .byWords)
