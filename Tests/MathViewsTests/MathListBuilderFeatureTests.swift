...
struct MathListBuilderFeatureTests {
    ...
    @Test func displayStyle() throws {
        ...
        for (latex, desc) in testCases {
            ...
            if list == nil || error != nil {
                return // Skipped: ("\\displaystyle/\\textstyle not implemented: \(desc). Error: \(error?.localizedDescription ?? "nil result")")
            }

            let unwrappedList = try #require(list, "Should parse: \(desc)")
            #expect(unwrappedList.atoms.count >= 1, "\(desc) should have atoms")
        }
    }

    @Test func middleDelimiter() throws {
        ...
        for (latex, desc) in testCases {
            ...
            if list == nil || error != nil {
                return // Skipped: ("\\middle not implemented: \(desc). Error: \(error?.localizedDescription ?? "nil result")")
            }
    ...
    @Test func substack() throws {
        ...
        for (latex, desc) in testCases {
            ...
            let unwrappedList = try #require(list, "Should parse: \(desc)")
            #expect(error == nil, "Should not error on \(desc): \(error?.localizedDescription ?? "none")")
            #expect(unwrappedList.atoms.count >= 1, "\(desc) should have atoms")
    ...
    @Test func manualDelimiterSizing() throws {
        ...
        for (latex, desc) in testCases {
            ...
            if list == nil || error != nil {
                return // Skipped: ("Manual delimiter sizing (\\big, \\Big, \\bigg, \\Bigg) not implemented: \(desc). Error: \(error?.localizedDescription ?? "nil result")")
            }
    ...
    @Test func spacingCommands() throws {
        ...
        for (latex, desc) in testCases {
            ...
            if list == nil || error != nil {
                return // Skipped: ("Spacing commands (\\,, \\:, \\;, \\!) not implemented: \(desc). Error: \(error?.localizedDescription ?? "nil result")")
            }
    ...
    @Test func multipleIntegrals() throws {
        ...
        for (latex, desc) in testCases {
            ...
            if let err = error {
                Issue.record("ERROR: \(err.localizedDescription)")
            } else if list == nil {
                Issue.record("List is nil but no error")
            }
    ...
    @Test func continuedFractions() throws {
        ...
        for (latex, desc) in testCases {
            ...
            if list != nil && error == nil {
                ...
                return // Skipped: ("\\cfrac may have issues: \(desc). Error: \(error?.localizedDescription ?? "nil result")")
            }
        }
    }
    ...
    @Test func displayStyleFraction() throws {
        ...
        #expect(error == nil, "\(desc)")
        let unwrappedList = try #require(list, "\(desc)")
        #expect(unwrappedList.atoms.count == 1, "\(desc)")

        let frac = try #require(unwrappedList.atoms[0] as? Fraction, "\(desc)")
    ...
    @Test func displayAndTextStyleFractions() throws {
        ...
        #expect(error == nil, "\(desc)")
        let unwrappedList = try #require(list, "\(desc)")
        #expect(unwrappedList.atoms.count >= 4, "Should have y, ', =, -, and fraction")
        ...
        #expect(error == nil, "Should parse nested dfrac/tfrac")
        #expect(nestedList != nil, "Should parse nested dfrac/tfrac")
    }
