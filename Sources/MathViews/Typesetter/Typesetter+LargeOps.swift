...
extension Typesetter {
    ...
    func makeLargeOp(_ largeOperator: LargeOperator!) -> Display? {
        ...
        if largeOperator.nucleus.count == 1 {
            var glyph = findGlyphForCharacterAtIndex(largeOperator.nucleus.startIndex, inString: largeOperator.nucleus)
            if glyph != 0 {
            ...
            let glyphDisplay = GlyphDisplay(glyph: glyph, range: largeOperator.indexRange, font: styleFont)
            ...
            let displayAtom = CTLineDisplay(
                attributedString: line, position: currentPosition, range: largeOperator.indexRange, font: styleFont,
                atoms: [largeOperator],
            )
    ...
    func makeLeftRight(_ inner: Inner?, maxWidth: CGFloat = 0) -> Display? {
        ...
        if let delimiterMultiplier = inner!.delimiterHeight {
            ...
            let axisDistance = (delta / 500) * Typesetter
                .kDelimiterFactor // This represents atleast 90% of the formula
            let paddingDistance = 2 * delta - Typesetter
