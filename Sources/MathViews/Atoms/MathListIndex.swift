...
public final class MathListIndex {
    ...
    var finalIndex: Int { subIndexType == .none ? atomIndex : subIndex?.finalIndex ?? 0 }
    ...
    func prevIndex() -> MathListIndex? {
        if subIndexType == .none {
            if atomIndex > 0 { return MathListIndex(level0Index: atomIndex - 1) }
        } else {
    ...
    func nextIndex() -> MathListIndex {
        if subIndexType == .none {
            MathListIndex(level0Index: atomIndex + 1)
        } else if subIndexType == .nucleus {
            MathListIndex(at: atomIndex + 1, with: subIndex, type: subIndexType)
        } else {
            MathListIndex(at: atomIndex, with: subIndex?.nextIndex(), type: subIndexType)
        }
    }
    ...
    func isAtSameLevel(with index: MathListIndex?) -> Bool {
        if subIndexType != index?.subIndexType {
            ...
            subIndex?.isAtSameLevel(with: index?.subIndex) ?? false
        }
    }

    /// Returns the type of the innermost sub index.
    func finalSubIndexType() -> MathListSubIndexType {
        subIndex?.subIndex != nil ? subIndex!.finalSubIndexType() : subIndexType
    }
    ...
    func hasSubIndex(ofType type: MathListSubIndexType) -> Bool {
        subIndexType == type ? true : subIndex?.hasSubIndex(ofType: type) ?? false
    }

    func levelUp(with subIndex: MathListIndex?, type: MathListSubIndexType) -> MathListIndex {
        subIndexType == .none
            ? MathListIndex(at: atomIndex, with: subIndex, type: type)
            : MathListIndex(
    ...
    func levelDown() -> MathListIndex? {
        if subIndexType == .none {
            nil
        } else if let subIndexDown = subIndex?.levelDown() {
            MathListIndex(at: atomIndex, with: subIndexDown, type: subIndexType)
        } else {
            MathListIndex(level0Index: atomIndex)
        }
    }
    ...
    public init(level0Index: Int) { atomIndex = level0Index }
...
extension MathListIndex: Equatable {
    public static func == (lhs: MathListIndex, rhs: MathListIndex) -> Bool {
        if lhs.atomIndex != rhs.atomIndex || lhs.subIndexType != rhs.subIndexType {
            false
        } else if rhs.subIndex != nil {
            rhs.subIndex == lhs.subIndex
        } else {
            lhs.subIndex == nil
        }
    }
