...
public enum ParseError: Error, Equatable {
    ...
    public var localizedDescription: String {
        switch self {
            case let .mismatchBraces(message), let .invalidCommand(message), let .characterNotFound(message),
                 let .missingDelimiter(message), let .invalidDelimiter(message), let .missingRight(message),
                 let .missingLeft(message), let .invalidEnv(message), let .missingEnv(message),
                 let .missingBegin(message), let .missingEnd(message), let .invalidNumColumns(message),
                 let .internalError(message), let .invalidLimits(message):
                return message
        }
    }
