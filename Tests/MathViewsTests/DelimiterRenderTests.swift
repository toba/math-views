...
struct DelimiterRenderTests {
    ...
    static let delimiterSizeCases: [RenderCase] = [
        ...
        RenderCase(name: "Biggl_frac", latex: #"\Biggl( \frac{a}{b} \Biggr)"#),
        RenderCase(name: "auto_vs_manual", latex: #"\left( \frac{a}{b} \right) \quad \big( \frac{a}{b} \big)"#),
    ]

    @Test(arguments: delimiterSizeCases)
    func delimiterSizeRendering(_ renderCase: RenderCase) throws {
        let result = MathImageResult.useMathImage(latex: renderCase.latex, font: font, fontSize: fontSize)

        #expect(result.error == nil, "Should render \(renderCase.name) without error: \(result.error?.localizedDescription ?? "")")
    ...
    @Test func delimiterSizeProgression() throws {
        let font = MathFont.latinModernFont
        let fontSize: CGFloat = 24.0
        ...
        for command in sizeCommands {
            ...
            if let layoutInfo = result.layoutInfo {
                ...
                #expect(currentHeight > previousHeight,
                    "\\\(command) height (\(currentHeight)) should be greater than previous (\(previousHeight))")
                previousHeight = currentHeight
            }

            if let image = result.image, let imageData = image.pngData() {
                _ = saveImage(prefix: "delimiter", name: "progression_\(command)", pngData: imageData)
            }
        }
    }
}
