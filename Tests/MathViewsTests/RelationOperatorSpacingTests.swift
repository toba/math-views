...
struct RelationOperatorSpacingTests {
    ...
    @Test func relationOperatorSpacing() throws {
        let latex = "a + b = c"
        ...
        #expect(display != nil)
        #expect(
            try #require(display?.subDisplays.count) >= 5,
            "Should have at least 5 elements: a, +, b, =, c",
        )
        ...
        for (i, subdisplay) in try #require(display?.subDisplays.enumerated()) {
            ...
            if i > 0 {
                ...
                print(
                    "Element[\(i)] gap from previous: \(gap), x=\(currentX), width=\(subdisplay.width)",
                )
    ...
    @Test func binaryOperatorSpacing() throws {
        let latex = "a + b"
        let mathList = MathListBuilder.build(fromString: latex)
        #expect(mathList != nil)
        ...
        #expect(display != nil)
        #expect(try #require(display?.subDisplays.count) >= 3, "Should have at least a, +, b")
    }
}
