...
func checkAtomTypes(_ list:MathList?, types:[MathAtomType], desc:String) {
    if let list = list {
        ...
        #expect(types.count == 0, "MathList should have no atoms!")
    }
}
...
struct TestRecord: Sendable, CustomTestStringConvertible {
        ...
        init(build: String, atomType: [MathAtomType], types: [MathAtomType], extra: [MathAtomType] = [MathAtomType](), result: String) {
            self.build = build
    ...
    struct ParseErrorRecord: Sendable, CustomTestStringConvertible {
        ...
        static let parseErrorCases: [ParseErrorRecord] = [
                  ...
                  ParseErrorRecord(input: "\\left\\lmoustache 5 + 3 \\right)", expectedError: .invalidDelimiter),
                  ParseErrorRecord(input: "\\left(\\frac12\\right\\rmoustache + 3", expectedError: .invalidDelimiter),
                  ParseErrorRecord(input: "5 + 3 \\right)", expectedError: .missingLeft),
                  ParseErrorRecord(input: "\\left(\\frac12", expectedError: .missingRight),
                  ParseErrorRecord(input: "\\left(5 + \\left| \\frac12 \\right)", expectedError: .missingRight),
                  ParseErrorRecord(input: "5+ \\left|\\frac12\\right| \\right)", expectedError: .missingLeft),
                  ParseErrorRecord(input: "\\begin matrix \\end matrix", expectedError: .characterNotFound), // missing {
                  ParseErrorRecord(input: "\\begin", expectedError: .characterNotFound), // missing {
                  ParseErrorRecord(input: "\\begin{", expectedError: .characterNotFound), // missing }
                  ParseErrorRecord(input: "\\begin{matrix parens}", expectedError: .characterNotFound), // missing } (no spaces in env)
                  ParseErrorRecord(input: "\\begin{matrix} x", expectedError: .missingEnd),
                  ParseErrorRecord(input: "\\begin{matrix} x \\end", expectedError: .characterNotFound), // missing {
                  ParseErrorRecord(input: "\\begin{matrix} x \\end + 3", expectedError: .characterNotFound), // missing {
                  ParseErrorRecord(input: "\\begin{matrix} x \\end{", expectedError: .characterNotFound), // missing }
                  ParseErrorRecord(input: "\\begin{matrix} x \\end{matrix + 3", expectedError: .characterNotFound), // missing }
                  ...
                  ParseErrorRecord(input: "\\begin{matrix} \\notacommand \\end{matrix}", expectedError: .invalidCommand),
                  ParseErrorRecord(input: "\\begin{displaylines} x & y \\end{displaylines}", expectedError: .invalidNumColumns),
                  // eqalign/aligned now allow any number of columns (matching KaTeX)
                  // Only split is limited to max 2 columns
                  ParseErrorRecord(input: "\\begin{split} a & b & c \\end{split}", expectedError: .invalidNumColumns),
...
struct MathListBuilderTests {
    ...
    @Test func amsSymbBinaryOperators() throws {
        ...
        for (latex, name, expected) in testCases {
            ...
            #expect(error == nil, "Should not error on \(name): \(error?.localizedDescription ?? "none")")
            #expect(list != nil, "\(desc)")
    ...
    @Test func additionalTrigFunctions() throws {
        ...
        for func_ in functions {
            ...
            #expect(error == nil, "Should not error on \\\(func_): \(error?.localizedDescription ?? "none")")
            #expect(list != nil, "\(desc)")
            guard let list = list else { continue }
    ...
    @Test func fracInFrac() throws {
        ...
        #expect(list != nil, "\(desc)")
        #expect((list.atoms.count) == 1, "desc")
        var frac = list.atoms[0] as! Fraction
        #expect(frac.type == .fraction, "\(desc)")
        #expect(frac.nucleus == "", "\(desc)")
        #expect(frac.hasRule);
        ...
        #expect(subList != nil, "\(desc)")
        #expect((subList.atoms.count) == 1, "desc")
        frac = subList.atoms[0] as! Fraction
        #expect(frac.type == .fraction, "\(desc)")
        #expect(frac.nucleus == "", "\(desc)")
    ...
    @Test func sqrt() throws {
        ...
        #expect(list != nil, "\(desc)")
        #expect((list.atoms.count) == 1, "desc")
        let rad = list.atoms[0] as! Radical
        #expect(rad.type == .radical, "\(desc)")
        #expect(rad.nucleus == "", "\(desc)")
    ...
    @Test func sqrtInSqrt() throws {
        ...
        #expect(list != nil, "\(desc)")
        #expect((list.atoms.count) == 1, "desc")
        var rad = list.atoms[0] as! Radical
        #expect(rad.type == .radical, "\(desc)")
        #expect(rad.nucleus == "", "\(desc)")
        ...
        #expect(subList != nil, "\(desc)")
        #expect((subList.atoms.count) == 1, "desc")
        rad = subList.atoms[0] as! Radical
        #expect(rad.type == .radical, "\(desc)")
        #expect(rad.nucleus == "", "\(desc)")
    ...
    @Test func overInParens() throws {
        ...
        let frac = list.atoms[2] as! Fraction
        #expect(frac.type == .fraction, "\(desc)");
        #expect(frac.nucleus == "", "\(desc)");
        #expect(frac.hasRule);
        #expect(frac.rightDelimiter.isEmpty)
    ...
    @Test func overLine() throws {
        ...
        #expect(list != nil, "\(desc)");
        #expect((list.atoms.count) == 1, "desc");
        let over = list.atoms[0] as! OverLine
        #expect(over.type == .overline, "\(desc)");
        #expect(over.nucleus == "", "\(desc)");
    ...
    @Test func underline() throws {
        ...
        #expect(list != nil, "\(desc)");
        #expect((list.atoms.count) == 1, "desc");
        let under = list.atoms[0] as! UnderLine
        #expect(under.type == .underline, "\(desc)");
        #expect(under.nucleus == "", "\(desc)");
    ...
    @Test func accent() throws {
        ...
        #expect(list != nil, "\(desc)");
        #expect((list.atoms.count) == 1, "desc");
        let accent = list.atoms[0] as! Accent
    ...
    @Test func mathSpace() throws {
        ...
        #expect(list != nil, "\(desc)");
        #expect((list.atoms.count) == 1, "desc");
        let space = list.atoms[0] as! MathSpace
        #expect(space.type == .space, "\(desc)");
        #expect(space.nucleus == "", "\(desc)");
    ...
    @Test func mathStyle() throws {
        ...
        let style = list.atoms[0] as! MathStyle
        #expect(style.type == .style, "\(desc)");
        #expect(style.nucleus == "", "\(desc)");
        #expect(style.style == .text);
        
        let style2 = list.atoms[2] as! MathStyle
        #expect(style2.type == .style, "\(desc)");
        #expect(style2.nucleus == "", "\(desc)");
    ...
    @Test func matrix() throws {
        ...
        #expect(list != nil);
        #expect((list.atoms.count) == 1);
        let table = list.atoms[0] as! MathTable
        ...
        for i in 0..<2 {
            ...
            for j in 0..<2 {
                ...
                let style = cell.atoms[0] as! MathStyle
    ...
    @Test func pMatrix() throws {
        ...
        let table = innerList.atoms[0] as! MathTable
        #expect(table.type == .table);
        #expect(table.nucleus == "");
        #expect(table.environment == "matrix");
        #expect(table.interRowAdditionalSpacing == 0);
    ...
    func errors(_ testCase: ParseErrorRecord) throws {
        ...
        do {
            ...
            #expect(expectedCase.matches(error), "\(str): expected \(expectedCase) but got \(error)")
        }
    }
    ...
    @Test func custom() throws {
        let str = "\\lcm(a,b)";
        var error : NSError? = nil
        var list = MathListBuilder.build(fromString: str, error:&error)
        #expect(list == nil)
        #expect(error != nil)

        MathAtomFactory.add(latexSymbol: "lcm", value: MathAtomFactory.operatorWithName("lcm", limits:false))
        error = nil
        list = MathListBuilder.build(fromString: str, error:&error)
        let atomTypes = [MathAtomType.largeOperator, .open, .variable, .punctuation, .variable, .close]
    ...
    @Test func limits() throws {
        ...
        var str = "\\int";
        var list = MathListBuilder.build(fromString: str)!
        var desc = "Error for string:\(str)"

        #expect(list != nil, "\(desc)")
        #expect((list.atoms.count) == 1, "desc")
        var op = list.atoms[0] as! LargeOperator
        #expect(op.type == .largeOperator, "\(desc)")
        #expect(!(op.limits));
        ...
        // Int with limits
        str = "\\int\\limits"
        list = MathListBuilder.build(fromString: str)!
        desc = "Error for string:\(str)"

        #expect(list != nil, "\(desc)")
        #expect((list.atoms.count) == 1, "desc")
        op = list.atoms[0] as! LargeOperator
        #expect(op.type == .largeOperator, "\(desc)")
        #expect(op.limits)
    ...
    @Test func emptyInputs() {
        ...
        let result4 = MathListBuilder.build(fromString: "\\cfrac[")
