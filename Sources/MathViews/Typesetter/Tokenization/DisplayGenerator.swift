...
/// Converts fitted lines of ``BreakableElement`` values into ``Display`` objects for rendering.
///
/// After ``LineFitter`` splits elements into lines, this class produces the final display
/// tree. Each line becomes a ``MathListDisplay`` containing ``CTLineDisplay`` nodes for text
/// and pre-rendered ``Display`` nodes for complex structures. Lines are stacked vertically
/// with dynamic spacing based on actual content height.
final class DisplayGenerator {
    /// Minimum inter-line spacing as a fraction of font size (breathing room between lines).
    private static let minLineSpacingRatio: CGFloat = 0.2
    /// Minimum absolute line spacing as a multiple of font size (ensures readability).
    private static let minLineSpacingMultiplier: CGFloat = 1.2
    ...
    func generateDisplays(from lines: [[BreakableElement]], startPosition: CGPoint) -> [Display] {
        ...
        let minimumLineSpacing = font.fontSize * Self.minLineSpacingRatio
        ...
        for (index, line) in lines.enumerated() {
            ...
            if index < lines.count - 1 {
                ...
                let minSpacing = font.fontSize * Self.minLineSpacingMultiplier
