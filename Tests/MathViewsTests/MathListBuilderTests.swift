...
final class MathListBuilderTests: XCTestCase {
...
//    }
    ...
    func testCornerBracketDelimiters() throws {
        ...
        for (latex, name, expectedLeft, expectedRight) in testCases {
            ...
            XCTAssertNotNil(inner, "Should have Inner for \(name)")
    ...
    func testAdditionalTrigFunctions() throws {
        ...
        for func_ in functions {
            ...
            XCTAssertNotNil(op, "Should be LargeOperator for \\\(func_)")
    ...
    func testSqrtWithoutRadicand() throws {
        let str = "\\sqrt"
        let list = try XCTUnwrap(MathListBuilder.build(fromString: str))
        
        XCTAssertEqual(list.atoms.count, 1)
        let rad = try XCTUnwrap(list.atoms.first as? Radical)
        XCTAssertEqual(rad.type, .radical)
        XCTAssertEqual(rad.nucleus, "")
    ...
    func testVectorArrowCommands() throws {
        ...
        for cmd in commands {
            ...
            let accent = try XCTUnwrap(unwrappedList.atoms[0] as? Accent, "For \\\(cmd)")
    ...
    func testVectorArrowUnicodeValues() throws {
        ...
        for (cmd, expectedValue) in expectedUnicode {
            let str = "\\\(cmd){a}"
            let list = try XCTUnwrap(MathListBuilder.build(fromString: str))
            let accent = try XCTUnwrap(list.atoms[0] as? Accent)
    ...
    func testVectorArrowMultiCharacter() throws {
        ...
        for (cmd, content) in testCases {
            ...
            let accent = try XCTUnwrap(unwrappedList.atoms[0] as? Accent)
            let innerList = try XCTUnwrap(accent.innerList)
    ...
    func testVectorArrowLatexRoundTrip() throws {
        ...
        for (input, expected) in testCases {
            let list = try XCTUnwrap(MathListBuilder.build(fromString: input))
    ...
    func testMultiCharacterArrowAccentParsing() throws {
        ...
        for (latex, expectedContent, expectedUnicode) in testCases {
            ...
            let accent = try XCTUnwrap(unwrappedList.atoms[0] as? Accent, "Should be Accent for \(latex)")
    ...
    func testDisplayStyleFraction() throws {
        ...
        let frac = try XCTUnwrap(unwrappedList.atoms[0] as? Fraction, desc)
        XCTAssertEqual(frac.type, .fraction, desc)
    ...
    func testDisplayAndTextStyleFractions() throws {
        ...
        error = nil
        let nestedList = MathListBuilder.build(fromString: nestedStr, error: &error)
