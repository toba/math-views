...
struct MathFontTests {
    @Test func mathFontScript() {
        ...
        for font in MathFont.allCases {
            ...
            #expect(font.ctFont(withSize: CGFloat(size)).fontSize == CGFloat(size), "ctFont fontSize != size.")
            #expect(font.cgFont().postScriptName as? String == font.postScriptName, "cgFont.postScriptName != postScriptName")
            #expect(CTFontCopyFamilyName(font.ctFont(withSize: CGFloat(size))) as String == font.fontFamilyName, "ctfont.family != familyName")
        }
    ...
    @Test func onDemandMathFontScript() {
        ...
        #expect(mathFont.ctFont(withSize: CGFloat(size)).fontSize == CGFloat(size), "ctFont fontSize test")
    }
    ...
    @Test func fallbackFont() throws {
        ...
        let mathFont = try #require(FontManager().font(withName: MathFont.latinModernFont.rawValue, size: 20))
        mathFont.fallbackFont = systemCTFont

        var error: NSError?
        let mathList = MathListBuilder.build(fromString: "\\text{中文测试}", error: &error)

        #expect(error == nil, "Should parse Chinese text without error")
        #expect(mathList != nil, "Math list should be created")
