...
private func applyMap(_ character: Character, _ map: CharStyleMap) -> UTF32Char? {
    ...
    if character.isUpperEnglish, let rangeStart = map.upperEnglish {
        return rangeStart + (codePoint - Character("A").utf32Char)
    }
    if character.isLowerEnglish, let rangeStart = map.lowerEnglish {
        return rangeStart + (codePoint - Character("a").utf32Char)
    }
    if character.isCapitalGreek, let rangeStart = map.capitalGreek {
        return rangeStart + (codePoint - UnicodeSymbol.capitalGreekStart)
    }
    if character.isLowerGreek, let rangeStart = map.lowerGreek {
        return rangeStart + (codePoint - UnicodeSymbol.lowerGreekStart)
    }
    if character.isGreekSymbol, let rangeStart = map.greekSymbol {
        return rangeStart + character.greekSymbolOrder!
    }
    if character.isNumber, let rangeStart = map.number {
        return rangeStart + (codePoint - Character("0").utf32Char)
    }
    return nil
}
...
private func defaultStyleChar(_ character: Character) -> UTF32Char {
    if character.isLowerEnglish || character.isUpperEnglish || character.isLowerGreek
        || character.isGreekSymbol
    {
    ...
    switch fontStyle {
        ...
        case .boldItalic:
            ...
            return applyMap(character, boldItalicMap)
                ?? (character.isNumber
                    ? applyMap(character, boldMap) ?? character.utf32Char : character.utf32Char)
        case .calligraphic: return applyMap(character, calligraphicMap) ??
        defaultStyleChar(character)
