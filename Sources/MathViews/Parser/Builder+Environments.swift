...
extension MathListBuilder {
        ...
        if let accent = MathAtomFactory.accent(named: command) {
            ...
            if hasCharacters, string[currentCharIndex] == "[" {
                ...
                if hasCharacters {
                    ...
                    if alignmentCharacter == "l" || alignmentCharacter == "r" ||
                        alignmentCharacter ==
                        "c"
                    {
    ...
    mutating func readOptionalAlignment() -> ColumnAlignment? {
        ...
        switch alignmentCharacter {
            ...
            default:
                setError(
                    .invalidEnv(
                        "Invalid alignment specifier: \(alignmentCharacter). Must be l, c, or r",
                    ),
                )
                return nil
        }
    ...
    func assertNotSpace(_ character: Character) {
        assert(
            character >= "\u{21}" && character <= "\u{7E}",
            "Expected non-space character \(character)",
        )
    }
