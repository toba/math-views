...
/**
    Horizontal text alignment for `MathUILabel`.
 */
...
/** The main view for rendering math.
 ...
 `MathUILabel` accepts either a string in LaTeX or an `MathList` to display. Use
 `MathList` directly only if you are building it programmatically (e.g. using an
 ...
 */
@IBDesignable
public class MathUILabel : MathView {
        
    /** The `MathList` to render. Setting this will remove any
     `latex` that has already been set. If `latex` has been set, this will
     return the parsed `MathList` if the `latex` parses successfully. Use this
     setting if the `MathList` has been programmatically constructed, otherwise it
     is preferred to use `latex`.
     */
    public var mathList:MathList? {
        set {
            _mathList = newValue
            _error = nil
            _latex = MathListBuilder.mathListToString(newValue)
            self.invalidateIntrinsicContentSize()
            self.setNeedsLayout()
        }
        get { _mathList }
    }
    private var _mathList:MathList?
    ...
    private var _textColor:MathColor?
     ...
     */
    @IBInspectable
    public var contentInsets:MathEdgeInsets {
        set {
    ...
    private var _contentInsets = MathEdgeInsetsZero
    ...
    private var _labelMode = MathUILabelMode.display
    ...
    private var _textAlignment = MathTextAlignment.left
    
    /** The internal display of the MathUILabel. This is for advanced use only. */
    public var displayList: MathListDisplay? { _displayList }
    private var _displayList:MathListDisplay?
    ...
    public var errorLabel: MathLabel?
    ...
    func initCommon() {
        ...
        self.backgroundColor = MathColor.clear
        
        _textColor = MathColor.black
        let label = MathLabel()
    ...
    override public func draw(_ dirtyRect: MathRect) {
        ...
        // drawing code
        let context = graphicsGetCurrentContext()!
        context.saveGState()
    ...
    func _sizeThatFits(_ size:CGSize) -> CGSize {
        ...
        if debugLogging {
            print("\n=== MathUILabel intrinsicContentSize DEBUG ===")
                ...
                if latexLower.contains("\\begin{\(env)}") {
                    ...
                    if let ml = _mathList {
                        for atom in ml.atoms {
                            if let table = atom as? MathTable, table.numRows == 1 {
        ...
        if debugLogging {
            ...
            if let display = displayList {
                ...
                for (i, sub) in display.subDisplays.enumerated() {
                    ...
                    var textContent = ""
                    if let ctLineDisplay = sub as? CTLineDisplay,
                    ...
                    // Show internal structure for MathListDisplay
                    if let mathListDisplay = sub as? MathListDisplay, !mathListDisplay.subDisplays.isEmpty {
                        ...
                        for (j, innerSub) in mathListDisplay.subDisplays.enumerated() {
                            ...
                            if let innerCTLineDisplay = innerSub as? CTLineDisplay,
