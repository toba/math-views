...
/// The payload carried by a ``BreakableElement``, distinguishing simple text from
/// pre-rendered complex structures.
enum ElementContent {
  /// Simple text content
  case text(String)
...
/// A single unit in the line-breaking pipeline with pre-calculated dimensions and break rules.
///
/// The ``AtomTokenizer`` produces these from ``MathAtom`` values. Each element carries its
/// measured width, height, ascent/descent, and a break-quality score that guides ``LineFitter``
/// in choosing where to split lines. Elements with the same ``groupId`` (e.g. a base atom
/// and its scripts) are kept together on the same line.
