...
/// Character classification helpers for Unicode math symbol style mapping.
extension Character {
    /// The character's UTF-32 code point value (sum of all scalar values).
    var utf32Char: UTF32Char { unicodeScalars.map(\.value).reduce(0, +) }
    /// Whether the character is a lowercase ASCII letter (a–z).
    var isLowerEnglish: Bool { self >= "a" && self <= "z" }
    /// Whether the character is an uppercase ASCII letter (A–Z).
    var isUpperEnglish: Bool { self >= "A" && self <= "Z" }
    /// Whether the character is an ASCII digit (0–9).
    var isNumber: Bool { self >= "0" && self <= "9" }

    /// Whether the character is a lowercase Greek letter (α–ω).
    var isLowerGreek: Bool {
        let unicodeCharacter = utf32Char
        return unicodeCharacter >= UnicodeSymbol.lowerGreekStart && unicodeCharacter <= UnicodeSymbol.lowerGreekEnd
    }

    /// Whether the character is an uppercase Greek letter (Α–Ω).
    var isCapitalGreek: Bool {
        let unicodeCharacter = utf32Char
        return unicodeCharacter >= UnicodeSymbol.capitalGreekStart && unicodeCharacter <= UnicodeSymbol.capitalGreekEnd
    }

    /// The index of this character among the six Greek symbol variants (ϵ, ϑ, ϰ, ϕ, ϱ, ϖ),
    /// or `nil` if it is not one of them. Used to compute the styled code point offset.
    var greekSymbolOrder: UInt32? {
        ...
        if let position = index { return UInt32(position) }
        return nil
    }

    /// Whether the character is one of the six Greek symbol variants (ϵ, ϑ, ϰ, ϕ, ϱ, ϖ).
