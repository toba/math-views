...
struct ExamplesImageGenerationTests {
    ...
    static let exampleCases: [RenderCase] = [
        ...
        RenderCase(name: "operatorname", latex: #"\operatorname{argmax}_{x \in \mathbb{R}} f(x) = \operatorname*{lim}_{n \to \infty} a_n"#),
        RenderCase(name: "delimiter", latex: #"\Bigg( \bigg( \Big( \big( x \big) \Big) \bigg) \Bigg)"#),
        RenderCase(name: "boldsymbol", latex: #"\boldsymbol{\alpha} + \boldsymbol{\beta} = \boldsymbol{\gamma}"#),
    ...
    func generateExampleImages(_ renderCase: RenderCase) throws {
        ...
        #expect(lightResult.error == nil, "Failed to render '\(renderCase.name)' (light): \(lightResult.error?.localizedDescription ?? "")")
        ...
        #expect(darkResult.error == nil, "Failed to render '\(renderCase.name)' (dark): \(darkResult.error?.localizedDescription ?? "")")
        ...
        #expect(lightSaved && darkSaved, "Should save both light and dark images for '\(renderCase.name)'")
    }
}
