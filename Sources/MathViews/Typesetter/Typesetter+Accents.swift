...
extension Typesetter {
    ...
    /// Width multiplier tiers for wide accent variant selection, keyed by maximum character count.
    /// Sorted ascending; the first tier whose threshold ≥ charCount is used.
    private static let wideAccentVariantTiers: [(maxChars: Int, multiplier: CGFloat)] = [
        (2, 1.0), // smallest variant
        (4, 1.5), // medium variant
        (6, 2.0), // large variant
    ]
    /// Fallback multiplier for wide accents covering 7+ characters.
    private static let wideAccentMaxMultiplier: CGFloat = 2.5

    /// Extra width request multiplier for stretchy arrow accents.
    private static let stretchyArrowExtraWidthMultiplier: CGFloat = 1.1
    /// Arrow tip padding as a fraction of font size (~0.167em).
    private static let arrowPaddingFraction: CGFloat = 1.0 / 6.0
    /// Wide accent padding as a fraction of font size (~0.1em).
    private static let wideAccentPaddingFraction: CGFloat = 1.0 / 10.0

    /// Determines which glyph variant to use for a wide accent based on content length.
    /// Returns a multiplier for the requested width.
    ...
    func wideAccentVariantMultiplier(for accent: Accent) -> CGFloat {
        ...
        for tier in Self.wideAccentVariantTiers where charCount <= tier.maxChars {
            return tier.multiplier
        }
        return Self.wideAccentMaxMultiplier
    }
    ...
    func makeAccent(_ accent: Accent?) -> Display? {
        ...
        if isWideAccent {
            ...
            if accent.isStretchy {
                requestedWidth = accenteeWidth * Self.stretchyArrowExtraWidthMultiplier
            } else {
