...
struct OperatornameRenderTests {
    ...
    static let comparisonCases: [RenderCase] = [
        ...
        RenderCase(name: "compare_lim", latex: "\\lim_{n \\to \\infty} a_n = \\operatorname*{lim}_{n \\to \\infty} b_n"),
    ]

    @Test(arguments: operatornameCases)
    func operatornameRendering(_ renderCase: RenderCase) throws {
        ...
        if let image = result.image, let imageData = image.pngData() {
            _ = saveImage(prefix: "operatorname", name: "basic_\(renderCase.name)", pngData: imageData)
        }
    }

    @Test(arguments: operatornameStarCases)
    func operatornameStarRendering(_ renderCase: RenderCase) throws {
        let result = MathImageResult.useMathImage(latex: renderCase.latex, font: font, fontSize: fontSize)
        ...
        if let image = result.image, let imageData = image.pngData() {
            _ = saveImage(prefix: "operatorname", name: "star_\(renderCase.name)", pngData: imageData)
        }
    }

    @Test(arguments: comparisonCases)
    func operatornameComparisonWithBuiltIn(_ renderCase: RenderCase) throws {
        let result = MathImageResult.useMathImage(latex: renderCase.latex, font: font, fontSize: fontSize)
