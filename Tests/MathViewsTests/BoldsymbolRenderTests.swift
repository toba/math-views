...
struct BoldsymbolRenderTests {
    ...
    static let comparisonCases: [RenderCase] = [
        ...
        RenderCase(name: "alpha_comparison", latex: "\\mathbf{\\alpha} \\text{ vs } \\boldsymbol{\\alpha}"),
        RenderCase(name: "gradient", latex: "\\boldsymbol{\\nabla} f = \\mathbf{0}"),
    ]

    @Test(arguments: greekCases)
    func boldsymbolGreekRendering(_ renderCase: RenderCase) throws {
    ...
    func boldsymbolComparisonWithMathbf(_ renderCase: RenderCase) throws {
        let result = MathImageResult.useMathImage(latex: renderCase.latex, font: font, fontSize: fontSize)
