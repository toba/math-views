...
struct MathListBuilderFeatureTests {
        ...
        for (latex, desc) in testCases {
            ...
            for atom in list.atoms {
                ...
                if let subScript = atom.subScript {
                    for subAtom in subScript.atoms where subAtom.type == .table {
                        foundTable = true
                        break
                    }
                }
    ...
    @Test func spaceAliasesRoundTrip() throws {
        // Verbose space aliases should parse and round-trip to their canonical short forms
        let aliasTests: [(String, CGFloat, String)] = [
            ("\\thinspace", 3, "\\, "),
            ("\\medspace", 4, "\\> "),
            ("\\thickspace", 5, "\\; "),
            ("\\negthinspace", -3, "\\! "),
        ...
        for (input, expectedMu, expectedOutput) in aliasTests {
            let list = try MathListBuilder.buildChecked(fromString: input)
            #expect(list.atoms.count == 1, "\(input) should produce 1 atom")
            let space = try #require(list.atoms[0] as? MathSpace, "\(input) should be MathSpace")
            #expect(space.space == expectedMu, "\(input) should have space \(expectedMu)")

            let latex = MathListBuilder.mathListToString(list)
            #expect(latex == expectedOutput, "\(input) round-trip: got '\(latex)', expected '\(expectedOutput)'")
        }
    }

    @Test func mkernRoundTrip() throws {
        // \mkern with arbitrary mu value should round-trip
        let list = try MathListBuilder.buildChecked(fromString: "\\mkern6.0mu")
        #expect(list.atoms.count == 1)
        let space = try #require(list.atoms[0] as? MathSpace)
        #expect(space.space == 6.0)

        // Serializes back to \mkern since 6.0 isn't in the canonical mapping
        // (\ maps to ordinary, not MathSpace)
        let latex = MathListBuilder.mathListToString(list)
        #expect(latex == "\\mkern6.0mu")
    }

    @Test func mkernNegativeValue() throws {
        let list = try MathListBuilder.buildChecked(fromString: "\\mkern-2.5mu")
        #expect(list.atoms.count == 1)
        let space = try #require(list.atoms[0] as? MathSpace)
        #expect(space.space == -2.5)
    }

    @Test func mkernKnownValueUsesCanonical() throws {
        // \mkern3mu should round-trip to \, (the canonical form for 3mu)
        let list = try MathListBuilder.buildChecked(fromString: "\\mkern3mu")
        let latex = MathListBuilder.mathListToString(list)
        #expect(latex == "\\, ")
    }
    ...
    @Test func displayAndTextStyleFractions() throws {
        ...
        for atom in list.atoms where atom.type == .fraction {
            foundFraction = true
    ...
    @Test func starredMatrices() throws {
        ...
        for (latex, desc) in testCases {
            ...
            for atom in list.atoms {
                ...
                if atom.type == .inner, let inner = atom as? Inner,
