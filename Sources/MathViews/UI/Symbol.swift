...
extension Character {
    ...
    var isLowerGreek: Bool {
        ...
        return unicodeCharacter >= UnicodeSymbol.lowerGreekStart
            && unicodeCharacter <= UnicodeSymbol.lowerGreekEnd
    }

    /// Whether the character is an uppercase Greek letter (Α–Ω).
    var isCapitalGreek: Bool {
        let unicodeCharacter = utf32Char
        return unicodeCharacter >= UnicodeSymbol.capitalGreekStart
            && unicodeCharacter <= UnicodeSymbol.capitalGreekEnd
    }
